...
export async function POST(nextRequest: NextRequest) {
  ...
  try {
    ...
    if (rows[0]) {
      ...
      const data = JSON.stringify({ ...json, whatsapp: json.whatsapp+1 });
      ...
      const data = JSON.stringify({ phonecall: 0, whatsapp: 1 });
      await db
        .insert(history)
        .values({ month, data });
    }
    ...
    console.log(`[DEBUG][API][POST][whatsapp] ${time}ms.`);
    return NextResponse.json({ status: 204 });
  } catch (err) {
    const time = Date.now() - currentTime;
    console.log(`[ERROR][API][POST][whatsapp] ${time}ms.`, err);
